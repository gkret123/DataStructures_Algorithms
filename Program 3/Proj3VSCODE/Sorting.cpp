...
//Comparison function for T3: compares two data objects by their last name, first name, and social security number if they have the same first and last name.
bool t3_comp(const Data* PersonA, const Data* PersonB){
    if((PersonA -> lastName == PersonB -> lastName) && (PersonA -> firstName == PersonB -> firstName) == true){
        return PersonA->ssn.compare(PersonB->ssn) < 0;
    }
}
// Checks if the next Data object in list has the same first and last names as the current Data object.
bool names_equal(const Data* PersonA, const Data* PersonB){
    if((PersonA -> lastName == PersonB -> lastName) && (PersonA -> firstName == PersonB -> firstName)){
        return true;
    }
    else{
        return false;
...
//Compare function for built in sorts
bool comp_all(const Data* person1, const Data* person2) {
...
//Compare Function to be used for t4 input 
bool ssn_comp(const Data* person1, const Data* person2) {
    if (person1 -> ssn >= person2 -> ssn)
        return false;
    else
        return true;
...
void sortDataList(list<Data *> &list) {
    ...
    //T4 sort: only ssn
    if((list.front()->lastName == list.back()->lastName) && (list.front()->firstName == list.back()->firstName) ){ 
        list.sort(ssn_comp); 
        return;
    }
    
  //Adds each object into the map and grouped by first name.
    for (auto i = list.begin(); i != list.end(); i++) {
        map_of_firstname[(*i)->firstName].push_back(*i);
    }

    //Map Data Objects to Last Names based on the order of the First Names from a previously created mapping of Data Objects to First Names
    for (auto i = 0; i < 500; i++) {
        auto p = map_of_firstname[first_names[i]];
        for (auto j = p.begin(); j != p.end(); j++) {
            map_of_lastname[(*j)->lastName].push_back(*j);
        }
    }
    auto listAdd = list.begin();
    //Iterating through all the Last Names in the Map of Last Names
    for (int i = 0; i < 500; i++) {
        auto p2 = map_of_lastname[last_names[i]];
        
        int z = p2.size() - 1; 
        for (int j = 0; j <= z; j++) {
                // Check if the first name of the current and next Data objects are the same
            if (j != z && p2[j]->firstName == p2[j + 1]->firstName) {
                Data *array_of_same_first_last [500];
                int position = 0;
                int k = j;
                array_of_same_first_last[position] = p2[j]; 
                position++;

                while(j < z && p2[j + 1]->firstName == p2[k]->firstName) {
                    j++;
                    array_of_same_first_last[position] = p2[j];
                    position++; 
                }
               
                sort(array_of_same_first_last,&array_of_same_first_last[position],ssn_comp);
                //Add sorted values from the array to the list, updating the returned list
                for (int w = 0; w < position; w++){
                    *listAdd = array_of_same_first_last[w];
                    listAdd++;
                }
            }
            //If the first name of the current Data object is not the same as the next Data object, add the current Data object individually
            else{
                *listAdd = p2[j];
                listAdd++;
            }
