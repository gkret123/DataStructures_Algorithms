...
void radixSort(std::list<Data *> &dataList) {
    ...
    for (auto data : dataList) {
        ...
        firstPassBins[binIndexFirstPass].emplace_back(data);
    }
    ...
    for (auto &bin : firstPassBins) {
        for (auto data : bin) {
            ...
            secondPassBins[binIndexSecondPass].emplace_back(data);
        }
    }
...
void sortByName(std::list<Data *> &l) {
  for (auto i : l) {
    ...
    if (lastIter != lastNamesMap.end() && firstIter != firstNamesMap.end()) {
      ...
      if (lastIndex >= 0 && lastIndex < 500 && firstIndex >= 0 &&
          firstIndex < 494) {
        sortedByName[lastIndex][firstIndex].emplace_back(
            i);  // Add the element to the list
      }
    }
  }
  ...
  for (int p1 = 0; p1 < 500; p1++) {
    for (int p2 = 0; p2 < 494; p2++) {
      ...
      l.splice(l.end(), sortedByName[p1][p2]);  // Merge the sorted sublist back
